...
export function LogsFilters({ filters, applications, events, onChange }: LogsFiltersProps) {
  return (
    <section className="pane p-3 grid grid-cols-1 lg:grid-cols-[minmax(18rem,1fr)_repeat(3,minmax(10rem,12rem))] gap-2">
      <label className="relative min-w-0">
        ...
        <input
          ...
          className="w-full bg-[#0a0c10] border border-[#30363d] pl-9 pr-3 py-2 font-mono text-xs text-[#e2e2e8] placeholder:text-[#849495] focus:outline-none focus:border-[#00f0ff]"
        />
      </label>
  ...
  return (
    <label className="flex flex-col gap-1 min-w-0">
      ...
      <select
        ...
        className="bg-[#0a0c10] border border-[#30363d] px-2 py-2 font-mono text-xs text-[#e2e2e8] focus:outline-none focus:border-[#00f0ff]"
      >
        {options.map((option) => (
